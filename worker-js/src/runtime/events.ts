...
export async function writeEvent(
  ...
  while (attempt < maxRetries) {
    try {
      if (!deps.pool) {
        deps.logger.warn(
          "Database pool is not available, skipping event write."
        );
        return;
      }
      ...
      attempt++;
      deps.logger.warn(
        `writeEvent failed (attempt ${attempt}): ${String(err)}`
      );
...
export async function readEvents(
  ...
  if (!deps.pool) {
    deps.logger.warn("Database pool is not available, skipping event read.");
    return [];
  }
  ...
  return res.rows.map((r: { event: string; data: any; timestamp: string }) => ({
    event: r.event,
    data: r.data,
...
export async function streamRunEvents(
  ...
  const checkEnd = () => {
    if (runContext.status !== "running") {
      const endPayload = {
        event: runContext.status,
        data: { runId: runContext.runId },
      };
