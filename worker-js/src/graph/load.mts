...
// 全局缓存：已编译图或工厂函数
const GRAPHS: Record<string, GraphInstance | ((config: any) => Promise<GraphInstance> | GraphInstance)> = {};
const GRAPH_SPEC: Record<string, { sourceFile: string; exportSymbol?: string }> = {};

// 从环境变量读取默认运行配置（与 orchestrator 保持一致）
const ENV_CONFIG: Record<string, any> | undefined = (() => {
  try {
    return process.env.LANGGRAPH_CONFIG ? JSON.parse(process.env.LANGGRAPH_CONFIG) : undefined;
  } catch {
    return undefined;
  }
})();
...
export async function registerFromEnv(
  ...
  for (const [graphId, rawSpec] of Object.entries(specs)) {
    const { resolved, sourceFile, exportSymbol } = await resolveGraph(rawSpec, options);
...
export async function getGraph(
  ...
  try {
    ...
    const { resolved } = await resolveGraph(defaultSpec, { cwd: options?.cwd ?? process.cwd() });
    GRAPHS[graphId] = resolved as any;
    ...
    throw new HTTPException(404, { message: `Graph \"${graphId}\" 加载失败：${(error as Error).message}` });
  }
}

export function assertGraphExists(graphId: string) {
  if (!GRAPHS[graphId]) throw new HTTPException(404, { message: `Graph \"${graphId}\" 未注册或不可用` });
}
